...
#setwd("/private/groups/migalab/emxu/scripts")
...
# Use the first and second argument
input_arg <- args[1]
output_arg <- args[2]

#load NucFreq bed file
df = fread(input_arg, stringsAsFactors = FALSE, fill=TRUE, quote="", header=FALSE, skip=2)
...
#print the table
#write.table(het_df_filtered, "all.hets.lessExpanded.greaterThan10.tbl", row.names = F, quote = F, sep="\t")
